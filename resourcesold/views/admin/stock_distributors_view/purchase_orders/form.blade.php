...
@section('page-back-button'){{ route("admin.stock_distributors_view") }}@endsection
...
<div class="form-container content-width-medium purchase-orders-form-content js-ak-delete-container" data-delete-modal-action="{{route("admin.po_distributors_view.purchase_orders.delete")}}">
    <form method="POST" action="@isset($data->id){{route("admin.stock_distributors_view.purchase_orders.update", $data->id)}}@else{{route("admin.stock_distributors_view.purchase_orders.store")}}@endisset" enctype="multipart/form-data" class="form-page validate-form" novalidate>
        <div hidden>
        ...
        @includeIf("admin.admin_layout.partials.form.footer",["cancel_route"=>route("admin.stock_distributors_view")])
    </form>
</div>
@isset($data->id)
