                ...
                <div class="row-100 el-box-select-search">
                    <div class="input-container">
                        ...
                        {{-- <div class="input-data">
                            ...
                            <div class="input-data">
    <select class="form-select" id="category" name="country_id">
        <option value="">{{ trans('admin/form.select') }}</option>

        @php
            $selectedCountryId = old('country_id') ?? ($data->country_id ?? null);
        @endphp

       @foreach($countries as $row)
    <option value="{{ $row->id }}" {{ $selectedCountryId == $row->id ? 'selected' : '' }}>
        {{ $row->countryname }}
    </option>
@endforeach
