...
*/
namespace App\Models\Admin\Userloginsession;
...
class Userloginsession extends Model
{
    ...
    {
        if ($request->has('userloginsession_sort_by') && $request->userloginsession_sort_by) {
            if($request->userloginsession_direction == "desc"){
                $query->orderByDesc($request->userloginsession_sort_by);
            } else {
                $query->orderBy($request->userloginsession_sort_by);
            }
        } else {
