...
class AdminRoles extends Model
{
    ...
    {
        return $this->belongsToMany(AdminUsers::class, 'admin_user_role', 'admin_roles_id', 'admin_users_id');
    }
    ...
    {
        if ($request->has('admin_roles_sort_by') && $request->admin_roles_sort_by) {
            if($request->admin_roles_direction == "desc"){
                $query->orderByDesc($request->admin_roles_sort_by);
            } else {
                $query->orderBy($request->admin_roles_sort_by);
            }
        } else {
            $query->orderBy("id");
        }
