...
class AdminPermissions extends Model
{
    ...
    {
        if ($request->has('admin_permissions_sort_by') && $request->admin_permissions_sort_by) {
            if($request->admin_permissions_direction == "desc"){
                $query->orderByDesc($request->admin_permissions_sort_by);
            } else {
                $query->orderBy($request->admin_permissions_sort_by);
            }
        } else {
            $query->orderBy("title")->orderBy("permission_slug");
        }
    }
